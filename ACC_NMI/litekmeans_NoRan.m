function [label, center, bCon, sumD, D] = litekmeans_NoRan(X, k, varargin)
...
for t=1:reps
    switch start
        case 'sample'
             %下近两行，均匀取1~n中k个整数。x0,x0+d,…,x0+(k-1)d.
             % 设x0=1，由1+(k-1)d<n确定d=floor((n-1)/(k-1)).
             %最小与1，最大与n两距离相近，即x0-1≈n-(x0+(k-1)d)定x0=floor(0.5(n-(k-1)d-1))+1
            step_d = floor((n-1)/(k-1));
            center = X([floor(0.5*(n-(k-1)*step_d-1))+1:step_d:n]',:);
   %         center = X(randsample(n,k),:);
        case 'cluster'
            step_d = floor((n-1)/(floor(.1*n)-1));
            Xsubset = X([floor(0.5*(n-(floor(.1*n)-1)*step_d-1))+1:step_d:n]',:);
